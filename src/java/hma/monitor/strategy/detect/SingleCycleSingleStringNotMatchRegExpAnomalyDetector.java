 ...
 *
 */
public class SingleCycleSingleStringNotMatchRegExpAnomalyDetector extends
	 ...
	 * @param pattern
	 */
	public SingleCycleSingleStringNotMatchRegExpAnomalyDetector(
	...
	protected AnomalyDetectionResult visitMonitorStrategyData(
		...
		boolean anomalous =
			!(p.matcher(strategyData.getData()).find());
		if (anomalous) {
			alarmLevel = MonitorStrategyAttachedTaskAdapter.ALARM_LEVEL_WARN;
			alarmInfo = 
				getMonitorItemName() + " NOT MATCH '" + getPattern() + "'\n" + 
				"[" + dateFormat.format(new Date(timestamp)) + "] " +
				getMonitorItemName() + " :\n" + strategyData.getData();
		}
