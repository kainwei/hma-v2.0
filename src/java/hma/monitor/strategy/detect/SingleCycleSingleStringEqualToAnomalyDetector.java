 ...
 *
 */
public class SingleCycleSingleStringEqualToAnomalyDetector extends
	 ...
	 * @param pattern
	 */
	public SingleCycleSingleStringEqualToAnomalyDetector(String monitorItemName,
			String pattern) {
	...
	protected AnomalyDetectionResult visitMonitorStrategyData(
		...
		if (anomalous) {
			...
			alarmInfo = 
				getMonitorItemName() + " EQUALS TO '" + getPattern() + "'\n" + 
				"[" + dateFormat.format(new Date(timestamp)) + "] " +
				getMonitorItemName() + " :\n" + strategyData.getData();
		}
